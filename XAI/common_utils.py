...
common_utils.py
----------------
Utilidades compartidas por los notebooks de clasificación de ruido mixto
...
def mostrar_reconstrucciones(model, dataset, n=8, device="cpu"):
    ...
    for i in range(n):
        axes[0, i].imshow(noisy_np[i].squeeze(), cmap="gray")
        axes[0, i].axis("off")

        axes[1, i].imshow(clean_np[i].squeeze(), cmap="gray")
        axes[1, i].axis("off")
